...
\item replace products to fit in a specific budget 
...
\item inside each category, different levels of sub-categories (alcohol, wine, red wine) 
\item after a couple of categorization levels (usually between 1 and 5), a list of products
\item by clicking on each product, ``product pages '' where detailed information (description, energetic values, price\ldots{}) is displayed
\end{itemize}
...
\\
\\
The second step consists in scrapping the relevant part of the html code
...
and cleaned the data at the same time and built a csv file to store it.
\\ 
...
To build this distance function, we started with distance between identical fields. This could be very simple for numeric fields such as weight/volume but is more complex for Strings. We will focus on distance between Strings for this project.
...
\\
\textit{Assortiment de chocolats au lait fourres et de biscuits enrobes de chocolat au lait.}
\\
becomes:
\\
\textit{assortiment chocolats lait fourres biscuits enrobes chocolat lait}
\\
In Python, we have:
...
The idea here is to be able to count words with the same root (eg chocolat, chocolats et chocolatée) as common words. To do so, we extract the root using FrenchStemmer  from nltk.stem.snowball library. We, then, replace the word by its root.
\\
Our example becomes:
\\
\textit{assorti chocolat lait fourr biscuit enrob chocolat lait}
\\
\\
...
We chose 6 initial products from the Simply inventory but in different product categories.
\\
...
First, we compute the distance between each product of our Gold Standard for the fields "nom", " ingredient" and "description". 
The results are displayed below:
\begin{figure}[!h]
\includegraphics[width=20cm]{/home/anthony/Documents/ProjetDataMining/crawlers/Results Monop/CalculDistance/results_images/MatDist_nom_1/Diapositive1.png}
\end{figure}
\\
\\
...
\begin{figure}[!h]
\includegraphics[width=20cm]{/home/anthony/Documents/ProjetDataMining/crawlers/Results Monop/CalculDistance/results_images/MatDist_ingredients_1/Diapositive1.png}
\end{figure}
\\
\\
\\
...
\begin{figure}[!h]
\includegraphics[width=20cm]{/home/anthony/Documents/ProjetDataMining/crawlers/Results Monop/CalculDistance/results_images/MatDist_descriptif_1/Diapositive1.png}
\end{figure}
\\
\\
\\
...
\begin{table}[htbp]
  \centering
    ...
    Match & Lingettes epaisses sans alcool pour bebe a leau nettoyante, a laloe vera  & Monoprix & BoutChou \\
    Match & Lingettes epaisses sans alcool pour bebe a leau nettoyante, a laloe vera  & Monoprix & BoutChou \\
    Match & Lingettes epaisses sans alcool pour bebe a leau nettoyante, a laloe vera  & Monoprix & BoutChou \\
    Match & Lingettes epaisses sans alcool pour bebe a leau nettoyante, a laloe vera  & Monoprix & BoutChou \\
    Match & Lingettes sensitives a laloe vera pour les peaux sensibles & Monoprix & BoutChou \\
    Match & Lingettes sensitives a laloe vera pour les peaux sensibles & Monoprix & BoutChou \\
...
\section{Conclusion}
This project was a very rich experience for us. We went through the different phases and had to find our way through technical difficulties especially in the crawling phase. Our first results seem encouraging and can consitute the first brick of a bigger project. The distance function can be improved by using more powerful semantic tools (LSA, LDA). We can also imagine to feed our algorithm with true shopper experiences in order to personnalize the substitution relevancy.    
